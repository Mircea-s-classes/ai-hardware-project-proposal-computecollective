...
FIX: Corrected the EMOTIONS list order to resolve the issue where only
     "Sad" was being detected due to a mismatch with the model's output.
...
# Emotion classes - This order is typically correct for FER2013 models
# The original list likely had an index mismatch, causing a misread of the output vector.
# Try this order first: Angry, Disgust, Fear, Happy, Neutral, Sad, Surprise
...
def run(camera_index, model_path, display=True):
    ...
    # Check if the cascade file exists and provide better error handling
    ...
    # Set frame resolution (optional improvement for stability)
    ...
    while True:
        ...
        # Use color frame for display, but gray for detection (performance optimization)
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

        # Detect faces (tuned parameters: scaleFactor=1.3, minNeighbors=5)
        ...
        for (x, y, w, h) in faces:
            # Extract the BGR face image slice
        ...
        if processed_faces:
            ...
            # Predict once for all faces in the current frame (batch prediction)
            ...
            for i, (x, y, w, h) in enumerate(face_rects):
                ...
                # Check bounds for safety, though it should be fine with 7 classes
    ...
    print("[INFO] GPU memory released (if applicable).")
