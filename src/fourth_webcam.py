...
NOTE: This file has been fixed to use batch prediction to improve
      real-time performance and prevent prediction lag, and has been
      updated to explicitly use the CPU for model operations to prevent
...
def run(camera_index, model_path, display=True):
    # --- Fix Setup: Define the device for memory-intensive operations ---
    # We default to CPU to solve the OOM issue, which is safer than relying on GPU memory
    device_name = "/cpu:0" 
    ...
    while True:
        ...
        # --- Batch Prediction for Real-time Stability ---
        ...
        if processed_faces:
            ...
            # --- FIX: Explicitly run prediction on the CPU/specified device ---
            with tf.device(device_name):
